...
struct ProgramNode : ASTNode {
    ...
    void dump(int indent = 0) const override {
        ...
        printIndent(indent + 1); std::cout << "Declarations:\n";
        for (auto& d : declarations) d->dump(indent + 2);
        printIndent(indent + 1); std::cout << "Statements:\n";
        for (auto& s : statements) s->dump(indent + 2);
    }
};
...
struct VarDeclNode : ASTNode {
    ...
    bool isStatic = false;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "VarDecl (";
        if (isStatic) std::cout << "static ";
        std::cout << typeName << " " << ident << ")\n";
        if (init) {
            printIndent(indent + 1); std::cout << "Init:\n";
            init->dump(indent + 2);
        }
    }
};
...
struct FuncDeclNode : ASTNode {
    ...
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "FuncDecl " << name << " : " << retType << "\n";
        printIndent(indent + 1); std::cout << "Params:\n";
        for (auto& p : params) { printIndent(indent + 2); std::cout << p.first << " " << p.second << "\n"; }
        printIndent(indent + 1); std::cout << "Body:\n";
        for (auto& b : body) b->dump(indent + 2);
        if (regressExpr) { printIndent(indent + 1); std::cout << "Regress:\n"; regressExpr->dump(indent + 2); }
    }
};
...
struct ClassDeclNode : ASTNode {
    ...
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Class " << name << "\n";
        for (auto& m : members) m->dump(indent + 1);
    }
};
...
struct ContractDeclNode : ASTNode {
    ...
    std::vector<ASTNodePtr> members;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Contract " << name << "\n";
        for (auto& m : members) m->dump(indent + 1);
    }
};
...
struct LiteralNode : ExprNode {
    std::string lit;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Literal: " << lit << "\n";
    }
};
...
struct IfNode : ASTNode {
    ...
    void dump(int indent = 0) const override {
        ...
        for (auto& e : elsi) {
            printIndent(indent + 1); std::cout << "Elsif Cond:\n"; e.first->dump(indent + 2);
            printIndent(indent + 1); std::cout << "Elsif Body:\n"; for (auto& s : e.second) s->dump(indent + 2);
        }
        if (!elseBranch.empty()) { printIndent(indent + 1); std::cout << "Else:\n"; for (auto& s : elseBranch) s->dump(indent + 2); }
    }
};
...
struct AssignNode : ASTNode {
    ...
    ExprNodePtr expr;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Assign: " << target << "\n";
        expr->dump(indent + 1);
    }
};
...
struct ShowNode : ASTNode {
    ...
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Show\n";
        for (auto& a : args) a->dump(indent + 1);
    }
};

struct DuringNode : ASTNode {
    ExprNodePtr cond; std::vector<ASTNodePtr> body;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "During\n";
        cond->dump(indent + 1);
        for (auto& s : body) s->dump(indent + 1);
    }
};

struct ForEachNode : ASTNode {
    std::string ident; ExprNodePtr toExpr; std::vector<ASTNodePtr> body;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "ForEach " << ident << "\n";
...
struct RepeatNode : ASTNode {
    std::vector<ASTNodePtr> body; ExprNodePtr untilExpr;
    void dump(int indent = 0) const override {
        printIndent(indent); std::cout << "Repeat\n";
        for (auto& s : body) s->dump(indent + 1);
        printIndent(indent); std::cout << "Until:\n";
