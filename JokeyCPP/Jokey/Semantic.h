...
public:
    ...
    void declare(const std::string& name, const Symbol& sym) {
        if (table.count(name))
            throw std::runtime_error("Erro: símbolo duplicado '" + name + "'");
        table[name] = sym;
    }
...
public:
    ...
    Symbol resolve(const std::string& name) {
        ...
        throw std::runtime_error("Erro: variável '" + name + "' não declarada");
    }
    ...
    void analyze(ASTNodePtr root) {
        ...
        // Declarações globais (vars, funcs, classes, contracts)
        ...
        if (auto v = dynamic_cast<VarDeclNode*>(node.get())) {
            ...
            if (v->init) {
                ...
                checkAssign(v->typeName, initType,
                    "Inicialização inválida da variável '" + v->ident + "'");
            }
        }
            ...
            if (f->regressExpr) {
                ...
                checkAssign(f->retType, ret,
                    "Retorno incompatível na função '" + f->name + "'");
            }

            popScope();
        }
        else if (dynamic_cast<ClassDeclNode*>(node.get())) {
            // placeholder simples
        }
        else if (dynamic_cast<ContractDeclNode*>(node.get())) {
            // placeholder simples
        }
    }
    ...
    void analyzeStmt(ASTNodePtr node) {
        if (auto a = dynamic_cast<AssignNode*>(node.get())) {
            ...
            checkAssign(s.type, exprType,
                "Atribuição inválida para '" + a->target + "'");
        }
            ...
            requireBool(cond, "Condição do 'during' deve ser booleana");
            ...
            if (listType.size() < 3 || listType.back() != ']')
                throw std::runtime_error("Expressão em foreach não é um vetor");
            ...
            requireBool(cond, "Condição do 'repeat' deve ser booleana");
        }
        else if (auto i = dynamic_cast<IfNode*>(node.get())) {
            std::string cond = analyzeExpr(i->cond);
            requireBool(cond, "Condição do 'if' deve ser booleana");
            ...
            for (auto& el : i->elsi) {
                ...
                requireBool(c, "Condição do 'elsif' deve ser booleana");
    ...
    std::string analyzeExpr(ExprNodePtr expr) {
        ...
        if (auto b = dynamic_cast<BinaryOpNode*>(expr.get())) {
            ...
            if (b->op == "+" || b->op == "-" || b->op == "*" || b->op == "/") {
                requireNumber(L, "Operador aritmético incompatível");
                requireNumber(R, "Operador aritmético incompatível");
                return "integer";
            }
            if (b->op == "==" || b->op == ">" || b->op == "<") {
                return "boolean";
